...
public class ConceptSet {
	 ...
	 * for this user.
	 */
	int[][] userConceptInclusionMatrix;// ��ϵ���� ��ά
	...
	public void readDataFromExcel() {
		...
		try {
			File file = new File("src/data/jester-standarlize.xls"); // �����ļ�����
...
//				System.out.println(Arrays.toString(tempNewConcept.users) + Arrays.toString(tempNewConcept.items));
	 ...
	 * @return The new concpet after replace
	 */
	 ...
	 * @param paraUserSet
	 * @param paraUser
	 * @return The result.0 menas not have,1 means have one,2 means the given user
	 *         is duplicate.
	 */
	 ...
	 * 
	 * @param paraConcept1
	 *            The first given concept.
	 * @param paraConcept2
	 *            The second given concept.
				...
				if ((tempNewConcept.users.length > 1) && (tempNewConcept.items.length > 0)) {
					...
					if (ratingMatrix.computeSimilarityOfConcepts(tempNewConcept) > ratingMatrix
						...
						if (count1 == 0) {
							...
							System.out.println("�����0����" + Arrays.toString(newConceptOfCross[count1].users)
							...
							for (int k = 0; k < count1; k++) {
								...
								if (isSame) {
									...
									if (k == count1 - 1) {
										...
										newConceptOfCross[count1] = tempNewConcept;
										System.out.println("�����" + Arrays.toString(newConceptOfCross[count1].users)
												+ Arrays.toString(newConceptOfCross[count1].items));
										count1++;
									} // of if
								} // of if
		...
		for (int i = 0; i < count1; i++) {
			if (newConceptOfCross[i].users.length <= paraLowUserThreshold) {
				...
				if (count2 == 0) {
					...
					System.out.println("�����0��: " + Arrays.toString(newConcept[count2].users)
					...
					for (int k = 0; k < count2; k++) {
						...
						if (isSame) {
							...
							if (k == count2 - 1) {
								// record the new concept
								newConcept[count2] = tempNewConcept;
								System.out.println("�����: " + Arrays.toString(newConcept[count2].users)
										+ Arrays.toString(newConcept[count2].items));
								count2++;
							} // of if
						} // of if
...
//					}//of if
	 ...
	 *            The recommendation in matrix, storing recommended concept index
...
//		System.out.println("FPR: " + FPR);
	...
	public void computeSparsity() {
		...
		System.out.println("���ݼ�ϡ��ȣ� " + sparsity);
	}//of computeSparsity
...
//		System.out.println("��������ʱ��1��" + (endTime1 - startTime1) + "ms");
//		
...
//		System.out.println("��������ʱ��2��" + (endTime2 - startTime2) + "ms");
